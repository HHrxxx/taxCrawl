...
for i in range(1, 10):
    # 爬分页数据
    url = "http://guizhou.chinatax.gov.cn/xwzx/jcsx/index_{}.html".format(i)
    r = requests.get(url, headers=hdrs)
    soup = BeautifulSoup(r.content.decode('utf8', 'ignore'), 'html.parser')
    # 获取指定ul 税务新闻模块
    div_list = soup.find('div', class_='NewsList')
    # print(ul_list)
    # 获取所有的li标签
    ...
    for item in li_list:
        ...
        div_list2 = soup2.find_all('div', class_='content')
        content = ""
        if (len(div_list2) == 0):
            break
        else:
        ...
        try:
            cur.execute(
                "insert into guizhou(Title,Url,Source,Content,PostTime,Status) values('%s','%s','%s','%s','%s','%s')" % (
                    data))
            db_conn.commit()
