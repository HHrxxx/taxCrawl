...
for i in range(1, 10):
    url = "http://beijing.chinatax.gov.cn/bjswjwz/xwdt/jcdt/index_{}.html".format(i)
    ur = "http://beijing.chinatax.gov.cn/bjswjwz/xwdt/jcdt/"  # 拼接文章详情链接
    ...
    for item in li_list:
        ...
        try:
            cur.execute(
                "insert into beijing(Title,Url,Source,Content,PostTime,Status) values('%s','%s','%s','%s','%s','%s')" % (
                    data))
            # 事物提交
            db_conn.commit()
